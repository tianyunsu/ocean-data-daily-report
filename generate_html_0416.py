...
# -*- coding: utf-8 -*-
"""
生成 2026-04-16 海洋AI技术日报 HTML
从 feishu_write_doc.py 读取 SECTIONS 数据，渲染成完整HTML
"""
import sys
import re
...
if not match:
    print("ERROR: 找不到 SECTIONS 变量", file=sys.stderr)
    sys.exit(1)
...
# ── 渲染各 section ─────────────────────────────────────────────
nav_html = ''.join(
    f'<a href="#section-{i+1}" class="nav-chip">{NAV_LABELS[i]}</a>'
...
for i, sec in enumerate(SECTIONS):
  ...
  <section class="section" id="section-{i+1}">
    <div class="section-header">
      ...
      <div>
        <div class="section-title">{title_cn}</div>
        <div class="section-en">{en_label}</div>
      </div>
      <div class="section-count">{cnt} 条</div>
    </div>
