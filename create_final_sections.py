...
final_sections = [
    ...
    {
        'title': '六、海洋数据管理与共享服务',
        'en': 'Ocean Data Management & Sharing Services / Geological Sample & Data Sharing',
        'items': [
            {
    ...
    {
        'title': '七、开放航次 / 船时共享',
        'en': 'Open Cruises / Ship Time Sharing / Shared Expedition',
        'items': [
            {
...
for i, section in enumerate(final_sections):
    ...
    sections_code += f"        ]\n"
    sections_code += f"    }},\n" if i < len(final_sections) - 1 else "    }\n"

sections_code += "]\n"

print(f"共 {len(final_sections)} 个方向，{sum(len(s['items']) for s in final_sections)} 条动态")

# 写入文件
with open('final_sections_code.py', 'w', encoding='utf-8') as f:
    f.write(sections_code)

print(f"\nSECTIONS代码已保存到: final_sections_code.py")