#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 2026-05-25 海洋AI日报 HTML"""

import sys
sys.path.insert(0, 'C:/Users/Administrator/WorkBuddy/Claw')

# 直接读取更新后的feishu_write_doc.py中的SECTIONS
exec(open('C:/Users/Administrator/WorkBuddy/Claw/feishu_write_doc.py', encoding='utf-8').read().split('def tr(')[0])
...
def badge_class(badge):
    if '近7天' in badge: return 'badge-new'
    if '1周' in badge: return 'badge-review'
    return 'badge-default'

nav_links = ''.join(
    f'<a href="#section-{i+1}" class="nav-chip">{ICONS[i]} {s["title"]}</a>'
...
for i, sec in enumerate(SECTIONS):
    ...
    for j, item in enumerate(sec['items']):
        ...
        badge_html = f'<span class="item-badge {bc}">{item.get("badge","")}</span>' if item.get('badge') else ''
        items_html += f'''
      <div class="item-card">
        <div class="item-title">
          {badge_html}
          <a href="{item['url']}" target="_blank" rel="noopener">{j+1}. {item['title']}</a>
        </div>
        <div class="item-abstract">{item['abstract']}</div>
        <div class="item-meta">
          <span class="meta-source">{item.get('source','')}</span>
          <span class="meta-date">{item.get('date','')}</span>
          <a href="{item['url']}" target="_blank" class="meta-link" rel="noopener">原文链接</a>
        </div>
      </div>'''
...
<div class="header">
  ...
  <div class="subtitle">报告日期：{DATE_CN} | 来源：中科院 / 信通院 / arXiv / ECMWF / DestinE / CMEMS / Schmidt Ocean / NOAA / JAMSTEC 等 | 自动生成</div>
...
<div class="footer">海洋AI技术日报 · {DATE_CN} · 自动生成 · WorkBuddy</div>
</body>
</html>'''

outpath = f'C:/Users/Administrator/WorkBuddy/Claw/daily_reports/海洋AI简报_{DATE_EN}.html'
