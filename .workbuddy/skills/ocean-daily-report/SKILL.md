...
description: >
  ...
  另含 **L3 前沿周报（含综述板块）**：当用户说"生成前沿周报"、"跑周报综述"、"出本周综述"、
  "注入综述"、"更新周报" 时使用同一条流水线（`weekly_report.py`），详见「L3 前沿周报与综述板块」。
agent_created: true
---
...
属端点级 WAF 拦截。**但该端点会间歇恢复**（2026-09-24 实测已恢复 200 并正常返回结果）：
`harvest.py` 已同时保留两条通道，**官方 API 可用时以其为主、OpenAlex 作补充**，两者互为备份，
不要因为一次 406 就永久废弃某条通道。预印本通道正常产出时每期应有 **300–500 条**；若为 0，说明通道失败，
须用 `--preprint-only` 补跑并在日志记录。OpenAlex 的 `locations` 还会附带**期刊版本**信息，
直接驱动"预印本按所属期刊等级排序"（见分级口径规则 4）。

**分批失败后的补跑（2026-09-24 实测踩坑，务必按此操作）**：OpenAlex 限流时 `harvest.py` 会出现
「部分方向成功、部分方向 429」的混合状态（本期 9 方向中 4/6/7/8/9 失败，且失败的 4/6/7/8 仍产出了预印本条目）。
- **`--dirs` 补跑会覆盖池文件**（写模式为 `"w"`），只跑失败方向会把已成功方向的结果**整文件丢掉**。
  正确做法：先 `cp data/pool/YYYY-MM-DD.jsonl data/pool/_bak_YYYY-MM-DD.jsonl` 备份 →
  `harvest.py --dirs <失败方向>` 补跑 → 用脚本按 `direction_id` 合并（成功方向取备份、失败方向取补跑结果）并去重。
- **`--resume` 不适用于此场景**：它把「池文件里已有任意条目」的方向视为已完成，
  而失败的 4/6/7/8 恰好都有预印本条目，会被整体跳过，补跑等于没跑。`--resume` 只适合「整轮中断、方向完全没跑」的情况。
- 补跑前先做一次连通性探测（单条 `/works?per-page=1` 请求），避免在限流期内空跑。
...
**如何触发（自动层 vs 深度层，务必分清）**

| 层 | 需要人/模型介入吗 | 触发方式 |
|----|------------------|----------|
| **自动综述** | 不需要 | 跑 `weekly_report.py` 即有，无人值守 |
| **深度综述** | **需要**——要有人读草稿并写解读 | ① 无人值守：每周日随当日日报流程执行（阶段四第 7 步），由 AI 读 `*.draft.md` 代笔；② 手动：苏老师说一句指令即可（见下表） |

**手动指令对照（任意一句都能命中，无需指定脚本路径）**

| 苏老师说 | 应执行的动作 |
|----------|--------------|
| 「生成本周前沿周报综述」/「跑周报综述」/「出本周综述」 | 跑脚本出草稿 → 读草稿写 `weekly/synth/YYYY-Www.md` → **跑链接核验（异常须 0）** → 再跑脚本注入 → `git add weekly/ && push` |
| 「生成周报综述，重点看 X（如海浪与海冰）」 | 同上，但综述的内容组织按指定重点倾斜（总体判断与线索部分体现） |
| 「周报只更新统计，不要深度综述」 | 只跑 `py -3 weekly_report.py --end <日期> --days 14`（自动综述层），不撰写、不覆盖已有 synth md |
| 「注入综述」（苏老师已自行编辑 `weekly/synth/*.md`） | 只跑脚本渲染该 md + 推送，**不得改写其内容** |
| 「周报综述写好了吗 / 周报在哪」 | 查 `weekly/YYYY-Www.html` 是否含 `synth/` 深稿（页面统计区会标注"深度综述/自动综述"） |

**降级行为（不会出错）**：`weekly/synth/YYYY-Www.md` 不存在时，页面顶部显示"本期未提供人工/模型撰写的深度综述"，
仅呈现自动综述层 —— 不会空白、不会报错，可随时补写后重跑脚本注入。
...
25. **`harvest.py` 分批失败后用 `--dirs` 补跑会静默丢掉已成功方向（2026-09-24 实测）**：OpenAlex 限流时会出现
    「部分方向成功、部分方向 429」的混合状态（本期 9 方向中 4/6/7/8/9 失败，且失败的 4/6/7/8 因预印本通道仍产出了条目）。
    此时若直接 `harvest.py --dirs 4,6,7,8,9` 补跑，**写模式为 `"w"` 会覆盖整个池文件**，已成功的 1/2/3/5 方向全部丢失；
    而 `--resume` 又因「该方向已有条目」把它们判为已完成而跳过——两种直觉做法都会导致数据缺失且不报错。
    - **铁律**：补跑前先 `cp data/pool/YYYY-MM-DD.jsonl data/pool/_bak_YYYY-MM-DD.jsonl`，
      补跑后按 `direction_id` 合并（成功方向取备份、失败方向取补跑结果）并去重，最后核对「合并总数 = 备份中保留方向数 + 补跑方向数」。
    - 补跑前先做连通性探测（单条 `/works?per-page=1`），避免在限流期空跑；
      OpenAlex/Crossref 限流期（503/429）**不要并发调用**，阶段五的举证与核验应串行并留间隔，必要时延后重试。

26. **飞书链路状态会变化，不要在日志里沿用上期结论**：文档 API 与机器人 webhook 是**两条独立链路**，
    token 也会被更换。2026-09-24 实测：机器人 webhook **首次发送成功**（`StatusCode: 0`），
    而文档 API 换成新 token `I0dGdyOwkoo9xQxebq3c3Lw9nrh` 后**仍 404**。
    - **铁律**：每期都要**实际执行**两条链路并**分别记录**结果（成功/失败 + 状态码），
      不得因为"上期 404"就跳过执行或直接写失败；二者均不阻断 GitHub Pages 发布。

27. **去重不能只比 URL：同一成果"往期用新闻稿链接、本期用 DOI 链接"会整条漏判（2026-09-24 实测）**：
    09-15 期收录了中科院海洋所「LeadNet 泛北极冬季冰间水道数据集」，用的是 **qdio 新闻页 URL**；
    09-24 期候选用了 **DOI 链接**（`10.1080/17538947.2026.2728309`），因此「本期 URL 与近 12 期 URL 求交集」
    的结果为 0，去重检查"通过"，但内容实为重复。
    - **铁律**：阶段五去重必须**双路并行**——① URL 集合差；② **成果名/标题关键词 + DOI 尾号**（如 `17538947.2026.2728309`、
      `120317`）在 `posts/` 全量 grep。两条都干净才可判定"无重复"。
    - 反之亦然：本期用新闻页、往期用 DOI 时同样漏判。**凡是"新闻稿 + DOI"并存的成果，两个标识符都要查。**

28. **`gen_html_*.py` 模板里的星期是硬编码的，必须按当日改写**：既有模板由 09-20（周日）派生，
    `date-line` 写死"· 周日"。2026-09-24 为**周四**，若不改写会直接对外发布错误星期。
    - **铁律**：复制上一期 `gen_html_*.py` 时，必须同时改写 `TODAY`、`TODAY_CN`、**星期**与页脚来源，
      并在阶段五增加一项肉眼核对：`date-line` 的日期与星期是否与当日一致。
    - 同类风险：模板中的 `item-badge`、来源声明、条目计数（"本期共 N 条动态"）均由数据驱动或硬编码，
      硬编码项一律列入复制后必改清单。
