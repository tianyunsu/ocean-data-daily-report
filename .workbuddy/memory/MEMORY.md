...
| **臆造 URL（按模式猜测）** | **严禁凭 URL 模式推断链接**。09-14 期出现两条 `mnr.gov.cn/dt/hy/202609/t20260911_2830895.html` / `_2830896.html`，是按自然资源部既有 URL 规则拼出来的，实测**两条均 404**，而该部网站根本没有这条稿件（同一栏目真实 ID 段为 2938xxx，非 2830xxx）。**铁律：每条 url 必须 urllib 实测返回 200，检索结果里没有的直接链接时，改用可访问的权威转载源并如实标注来源，绝不"造"URL** |
| **批量抓同一期刊整期导致跨期重复** | 按 MDPI 某一期（如 JMSE 14(16)）批量取稿极易撞历史条目。09-14 期从 JMSE 14(16) 取了 1453/1462 两条，**分别已在 08-18 期和 08-10 期收录**（且 08-21、08-31 两期还在"暂无新增"里明确写过"DTF-Net 已收录"）。**铁律：期刊整期批量取稿时，每个 DOI 尾号都要单独 grep `posts/`，不能只对新取的 URL 做一次集合比对** |
| **"已去重"自述不可信** | 另一台机器 09-14 日志自称"grep -rl posts/ 跨期去重：无重复""所有条目≥08-31（≤14天）"，实测**两项均不成立**（3 处重复、2 条为 08-07/08-08）。**铁律：跨机器接手时，不采信对方日志的质检结论，必须独立复算 URL 集合差与时效差** |
...
## 去重基准（滚动更新，只留最近 6 期）
> **≤09-03 的历史条目不再在此罗列**——`posts/` 目录本身就是全量存档，规则 4c 的 `grep -rl` 比读基准更可靠。本节只保留近 6 期用于快速人工比对，超出窗口一律 grep posts/。
...
- **09-14（16条+1备注[D9暂无新增]）**：KiloDA扩散模型0.24%格点重建公里尺度风场(arXiv 2609.11230, 09-10)、Ocean-E2E混合物理+数据驱动端到端神经同化极端海洋热浪预报(KDD 2026 AI4Science, 清华SAIL+崂山实验室, 会议日08-09；预印本2505.22071 v1=2025-05, 已按规则8补注)、高频雷达波高窗口包络统计(2609.10920, 09-10)、ML天气预报模型挪威北部站点评估(2609.10564, 挪威科大, 09-11)、扩散先验引导稀疏观测温度降尺度(2609.09247, 印度统计研究所, 09-09)、EMODnet全量数据集实现EDITO数据湖自动化同步(EMODnet Newsletter Summer 2026, 09-01)、WakeAtlas免费CesiumJS三维海洋探索平台(DEV Community/wakeatlas.uk, 09-12)、Sentinel-6B双星同轨串联飞行星间交叉校验·海平面基准卫星交接(NASA/JPL, 09-09)、CFOSAT SWIM海浪谱6.0.3/7.0.0/7.1.0三代版本与ERA5海况分级质量评估(《海洋学报》网络出版09-04, 南信大+国家卫星海洋应用中心)、MG-GCNN多粒度图神经网络三维海洋环境场重建(RS 18(17)3003, 西北工大, 09-04)、UN海洋十年社会经济数据共享指南(DCO-ODS, EMODnet参与, 09-10)、EMODnet Biology北海底拖网影响数据产品Bfiat模型(NIOZ, 09-11)、东海秋季共享航次向阳红18(自然资源部一所, 09-10~09-19)、西太平洋弧后盆地发现大型高温热液区(向阳红10, 清华+青岛海洋地质所+上海交大, 金15.4ppm/银1271ppm, 09-11)、雪龙2号为法国塔拉极地站破冰引航110海里(新华社, 09-11)、NOAA NCEI上线SOUP深海样品数据管理系统(09-10)。
  ⚠️ **本机质检修正（勿再犯）**：另一台机器本期**3处跨期重复**——①DTF-Net(JMSE 14(16)1453, 08-07)已于**08-18期**收录；②事件保留型浮标温度QC(JMSE 14(16)1462, 08-08)已于**08-10期**收录；③EMODnet数据吸纳服务新阶段(08-31)已于**09-04期**收录。三条均已替换为 Sentinel-6B 交叉校验 / CFOSAT SWIM 质量评估 / NOAA SOUP。另修复**2条臆造404链接**（`mnr.gov.cn/dt/hy/202609/t20260911_2830895.html`、`_2830896.html` 为按URL模式猜测、实测404，改指中新网与川观新闻）；EMODnet Biology 链接由站点首页改为具体数据产品页；Ocean-E2E 补注预印本原始公开日。剔除：OceanParcels v3.1.4(2025-08-07 >60天)、Fugro OCEANITY(重复于09-08)、WavyOcean 3.0(07-09 >60天)、Tsinghua DT review(01-2026 >60天)
